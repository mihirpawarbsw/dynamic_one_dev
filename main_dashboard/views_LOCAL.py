...
from login.views import copy_csv_to_named_folder
import traceback
############################################################################################
############################################################################################

def get_final_destination(user_id):
    """
    Generate the final destination folder path for a user-uploaded file.
    
    Args:
        user_id (str): User's user_id.

    Returns:
        str: The final destination folder path.
    """
    if not user_id:
        raise ValueError("User id is required")

    source, dest = settings.TEMP_UPLOAD, settings.TEMP_UPLOAD_COPIED
    # modified_email = f"temp_data_{user_email.replace('@', '_at_').replace('.', '_dot_')}"
    final_dest = f"{settings.TEMP_UPLOAD_COPIED}temp_data_{user_id}/"
    print(f"destination folder: {dest}\nfinal_dest_folder: {final_dest}")
    return final_dest
...
def bring_key_to_first(data, key):
    # Unpack the specified key first, then the rest of the dictionary
    return {key: data[key], **{k: v for k, v in data.items() if k != key}}
######################## ADDED ON 18-02-2025 ###############################
def bring_key_to_first_OLD_18022025(data, key):
...
def display_all_data1(request):
    if request.method == "POST":
        ...
        #source_folder = settings.TEMP_UPLOAD
        #destination_folder = settings.TEMP_UPLOAD_COPIED

        # Get the user by ID
        # user_email = request.session.get('email')
        #user_id = request.session.get('user_id')
        #message_resp = copy_csv_to_named_folder(source_folder,destination_folder,user_id)
        ############################ ADDED ON 26-03-2025 ##########################################
        # db_type = 'Sales'
        user_id = request.session.get('user_id')
        final_dest = get_final_destination(user_id)
        ...
        for file_name in [file for file in os.listdir(final_dest) if file.endswith('.csv')]:
...
def display_all_data(request):
    if request.method == "POST":
        ...
        df = pd.read_csv(final_dest + filename+'.csv',low_memory=False,engine='c')
...
def crosstab_table(request):
    ...
    if request.method == "POST":
        ...
        # final_row_col_array_grp_json = {'Sales_Marketwise_Q2_2024': ['Brand', 'Category']}
        print(f"*******************{final_row_col_array_grp_json}***************************")
        ...
        # filename = list(dict_table.keys())[0] + ".pkl"
        ...
        #######################################################################################
        df = pd.read_csv(final_dest + filename,low_memory=False,engine='c')
        # df = pd.read_csv(settings.TEMP_UPLOAD + filename,low_memory=False,engine='c')
        # df = pd.read_pickle(settings.TEMP_UPLOAD + filename)
        ...
        if (db_flag != 'Doors') or ((db_flag == 'Doors') and ('Door' not in selected_weight_column22)):
            df,selected_full_period_str,comparative_full_period_str = data_transformation(df,selected_weight_column22,selected_full_period,comparative_full_period,loop_vals_lst)
        ...
        #             cross_df = cross_df.swaplevel(i, i + 1, axis=1)

        #         cross_df = cross_df.sort_index(axis=1)
        #         ############################################
        #         # if total_levels == 2:
        ...
        #         print("+++++++++++++++++++measure_in_column++++++++++++++++++++++++")
        #         # a = [selected_weight_column_all, dict_table, df.head(), percent_calc, row_name, col_name,
        #         #                           data_type_resp,
        #         #                           seperated_flag_row, seperated_flag_col, totals_nested_flag,agg_func,
        #         #                           measure_row_column_position]
        #         # for i in a:
        #         #     print(i)
        ...
        # # Remove duplicate columns and index Anup
        # for axis in ['columns', 'index']:
        #     if getattr(cross_df, axis).duplicated(keep='first').any():
        #         cross_df = cross_df.loc[:, ~cross_df.columns.duplicated()] if axis == 'columns' else cross_df.loc[~cross_df.index.duplicated()]
        # ###### end ###########
        ...
        # last_level_values_list = []
        
        ################################ OLD CODE #####################################################
        # last_level_values_list = []
        # for measure in selected_weight_column22:
        #     if measure == 'Door':
        #         actual_val_measure = ' Door'
        #     elif measure == 'Unit':
        #         actual_val_measure = ' Units'
        #     else:
        #         actual_val_measure = ' Sales'

        #     value_list_metrics_page1 = [
        #             measure + '_' + selected_full_period_str + ' Rank',
        #             measure + '_' + selected_full_period_str + actual_val_measure,
        #             measure + '_' + comparative_full_period_str + actual_val_measure,
        ...
        #         ]
        #     for value_measure in value_list_metrics_page1:
        #         last_level_values_list.append(value_measure)
        # if brand_sales_index_value_flag == 'Yes':
        #     measure_bsindex = measure + '_' + selected_full_period_str + ' Brand sales Index'
        #     last_level_values_list.append(measure_bsindex)
        ...
        print('last_level_values_list_1408',last_level_values_list)
...
#             return [base_start + i * base_step + offset for i in range(length) for offset in (0, 2)]

#         # Determine the base_step based on conditions
#         if (measure_row_column_position == 'measure_in_column' 
#                 and len(row_name) == 1 
#                 and {"Brand", "Product Name"} & set(row_name)):
#             print('brand condition!')
#             base_step = 10  # For 'brand condition'
#         else:
#             print('not condition')
#             base_step = 9  # For 'not condition'
...
#         ##################### NEW LOGIC DYNAMIC SUBSET #######################################
        ...
        if (measure_row_column_position == 'measure_in_column') and (len(row_name) == 1) and ({"Brand"} & set(row_name)):
            print('brand condition!')
            base_start = 1
...
#         ##################### NEW LOGIC DYNAMIC SUBSET #######################################
        ...
        print(f"#############################{last_level_values_list}\n\n{sublist}\n\n{index_lst_cols}############################")
...
#             cross_df_graph = cross_df.droplevel(0, axis=0)
...
def crosstab_table_NEWLOGIC_WITHOUT_DF(request):
    ...
    if request.method == "POST":
        ...
        ############# REMOVED RANK - 23-09-2024 ############################################################
        if (measure_row_column_position == 'measure_in_column') and (len(row_name) == 1) and (("Brand" in row_name)):
            print('brand condition!')
            if len(selected_weight_column22) == 1:
                index_lst_cols = [1, 3]
            elif len(selected_weight_column22) == 2:
...
def crosstab_table_page2(request):
    ...
    if request.method == "POST":
        try:
            ...
            user_id = request.session.get('user_id')
            print("Function get_final_destination START...")
            final_dest = get_final_destination(user_id)
            print("Function get_final_destination END...")
            ############# added on 26-03-2025 #####################################################
            ...
            # print('seperated_flag_row_2 4107--',seperated_flag_row)
            # print('seperated_flag_col_2 4108--',seperated_flag_col)
            ...
            try:
                print("Function ensure_list START...")
                dict_base_filter_data = ensure_list(dict_base_filter_data)
                print("Function ensure_list END...")
            except:
                pass
            ...
            # ===============================Main logic start here============================= #
            ######################## CROSSTAB LOGIC #################################################
            print("Function create_selected_weight_column START...")
            selected_weight_column = create_selected_weight_column(Measure)
            print("Function create_selected_weight_column END...")
            ...
            unique_count_dict = {col: df[col].dropna().nunique() for col in categorical_cols}
            print('3244-unique_count_dict',unique_count_dict)
            ############################ FILL VALUES #########################################################
            ...
            if seperator_param in ["millions", "thousands"]:
               
                old_suffix, new_suffix = ("K", "M") if seperator_param == "millions" else ("M", "K")
               
                sales_columns = [col for col in selected_weight_column22 if f'Sales ({old_suffix}' in col]
                sales_columns = [col.replace(old_suffix, new_suffix) for col in sales_columns]

                selected_weight_column22 = [col.replace(f'Sales ({old_suffix}', f'Sales ({new_suffix}') for col in selected_weight_column22]
                df.rename(columns={col: col.replace(f'Sales ({old_suffix}', f'Sales ({new_suffix}') for col in df.columns}, inplace=True)

                facts_object_index_resp = {key.replace(f"Sales ({old_suffix}", f"Sales ({new_suffix}"): value
                                        for key, value in facts_object_index_resp.items()}
            ...
            ####################################################################################
            print("Function base_filter_resp START...")
            filter_dict_resp = base_filter_resp(df,dict_table,row_name,col_name)
            print("Function base_filter_resp END...")
            print("Function remove_duplicate_keys_and_values START...")
            filter_dict_resp = remove_duplicate_keys_and_values(filter_dict_resp)
            print("Function remove_duplicate_keys_and_values END...")
            ...
            print('44311111111-dict_table',loop_vals_lst)
            if (db_flag != 'Doors') or ((db_flag == 'Doors') and ('Door' not in selected_weight_column22)):
                print("Function data_transformation START...")
                df,selected_full_period_str,comparative_full_period_str = data_transformation(df,selected_weight_column22,selected_full_period,comparative_full_period,loop_vals_lst)
                print("Function data_transformation END...")
            elif ((db_flag == 'Doors') and ('Door' in selected_weight_column22)):
                df,selected_full_period_str,comparative_full_period_str = data_transformation_doors(filename,selected_weight_column22,selected_full_period,comparative_full_period,seperator_param)
            ...
            print("Function base_filter_resp_all START...")
            all_categories_vals = base_filter_resp_all(df,dict_table,row_name,col_name)
            print("Function base_filter_resp_all END...")
            ...
            print("Function base_filter_data START...")
            df = base_filter_data(df,dict_base_filter_data)
            print("Function base_filter_data END...")
            ...
            # print('dict_base_filter_data==',dict_base_filter_data)
            try:
                print("Function base_filter_data df_UNFILTERED START...")
                df_UNFILTERED = base_filter_data(df_UNFILTERED,filtered_dict_base_filter_data)
                print("Function base_filter_data df_UNFILTERED END...")
            except:
                pass
            ...
            #####################################################################################
            #####################################################################################
                def process_crosstab_logic(measure_position, weight_column, dict_table, df, percent_calc, col_name, row_name,
                                        data_type_resp, seperated_flag_col, seperated_flag_row, totals_nested_flag, agg_func):
                    """
                    Processes the crosstab logic for given input parameters and returns the processed dataframe.
                    """
                    print("Function sales_crosstab_logic_MAIN START...")
                    cross_df = sales_crosstab_logic_MAIN(weight_column, dict_table, df, percent_calc, col_name if measure_position == "measure_in_row" else row_name,
                                                        row_name if measure_position == "measure_in_row" else col_name, data_type_resp,
                                                        seperated_flag_col, seperated_flag_row, totals_nested_flag, agg_func, measure_position)
                    print("Function sales_crosstab_logic_MAIN END...")
                    total_levels = cross_df.columns.nlevels
                    # print('total_levels', total_levels)

                    for i in range(total_levels - 1):
                        cross_df = cross_df.swaplevel(i, i + 1, axis=1).sort_index(axis=1)
                    return cross_df
                ...
                if no_row_col_flag == 'row_col_present':
                    print("Function process_crosstab_logic row_col_present START...")
                    cross_df = process_crosstab_logic(measure_row_column_position, selected_weight_column_all, dict_table, df,
                                                    percent_calc, col_name, row_name, data_type_resp,
                                                    seperated_flag_row, seperated_flag_col,totals_nested_flag, agg_func)
                    print("Function process_crosstab_logic row_col_present END...")
                elif no_row_col_flag == 'no_row_col_present':
                    # print('row_name:', row_name, 'col_name:', col_name, 'measure_position:', measure_row_column_position)
                    print("Function single_dimension_logic START...")
                    cross_df = single_dimension_logic(df, row_name, col_name, seperated_flag_row, selected_weight_column_all, agg_func)
                    print("Function single_dimension_logic END...")
                # Transpose for row-based measures
                if measure_row_column_position == "measure_in_row":
                    cross_df = cross_df.T
     
                # Remove duplicate columns and index
                cross_df = cross_df.loc[:, ~cross_df.columns.duplicated(keep='first')]
                cross_df = cross_df[~cross_df.index.duplicated(keep='first')]
     
                # Renaming 'Grand Total' to 'Total (Among Displayed)'
                if display_grand_total_flag == 'gt_all' and ((len(col_name) == 0 and len(row_name) == 1) or (len(col_name) == 1 and len(row_name) == 1)):
                    if measure_row_column_position == 'measure_in_column':
                        filtered_df_gt = cross_df[cross_df.index.get_level_values(1) == 'Grand Total']
                        filtered_df_gt.index = pd.MultiIndex.from_tuples([
                            (filtered_df_gt.index[i][0], 'Total (Among Displayed)' if filtered_df_gt.index[i][1] == 'Grand Total' else filtered_df_gt.index[i][1])
                            for i in range(len(filtered_df_gt.index))
                        ])
                        ...
                        filtered_df_gt.columns = pd.MultiIndex.from_tuples([
                            (filtered_df_gt.columns[i][0], 'Total (Among Displayed)' if filtered_df_gt.columns[i][1] == 'Grand Total' else filtered_df_gt.columns[i][1])
                            for i in range(len(filtered_df_gt.columns))
                        ])
                ...
                # Process unfiltered data for 'Grand Total All Pivot'
                if len(row_name) > 0 and len(col_name) > 0:
                    print("Function process_crosstab_logic cross_df_UNFILTERED START...")
                    cross_df_UNFILTERED = process_crosstab_logic(measure_row_column_position, selected_weight_column_all, dict_table, df_UNFILTERED,
                                                                percent_calc, col_name, row_name, data_type_resp, seperated_flag_col,
                                                                seperated_flag_row, totals_nested_flag, agg_func)
                    print("Function process_crosstab_logic cross_df_UNFILTERED END...")
                elif no_row_col_flag == 'no_row_col_present':
                    print("Function single_dimension_logic cross_df_UNFILTERED START...")
                    ...
                    print("Function single_dimension_logic cross_df_UNFILTERED END...")
     
                if measure_row_column_position == "measure_in_row":
                    cross_df_UNFILTERED = cross_df_UNFILTERED.T
     
                cross_df_UNFILTERED = cross_df_UNFILTERED.loc[:, ~cross_df_UNFILTERED.columns.duplicated(keep='first')]
                cross_df_UNFILTERED = cross_df_UNFILTERED[~cross_df_UNFILTERED.index.duplicated(keep='first')]
     
                # Adding unfiltered grand totals to rows/columns
                if display_grand_total_flag == 'gt_all' and ((len(col_name) == 0 and len(row_name) == 1) or (len(col_name) == 1 and len(row_name) == 1)):
                    if measure_row_column_position == 'measure_in_column':
                        filtered_gt = cross_df_UNFILTERED[cross_df_UNFILTERED.index.get_level_values(1) == 'Grand Total']
                        cross_df = pd.concat([cross_df, filtered_gt], axis=0)
                    elif measure_row_column_position == 'measure_in_row':
                        filtered_gt = cross_df_UNFILTERED.loc[:, cross_df_UNFILTERED.columns.get_level_values(1) == 'Grand Total']
                        cross_df = pd.concat([cross_df, filtered_gt], axis=1)
                ...
                if measure_row_column_position == "measure_in_row":
                    ...
                    # exit('cross_dfrrr')
                    print("Function 2- derived_MAIN_fn STARTED!")
                    cross_df = derived_MAIN_fn(cross_df,base_sales_index_colname,row_name,col_name,measure_row_column_position,cagr_power_val,brand_sales_index_value_flag='No')
                    print("Function 2- derived_MAIN_fn END!")
                    cross_df = cross_df.T
                ...
                # ################## CODE TO REPLACE CY AND YA WITH ACTUAL PERIOD - 19-04-2024 ####
                print("Function 2- replace_cy_ya_with_actual_period STARTED!")
                cross_df = replace_cy_ya_with_actual_period(cross_df,measure_row_column_position,selected_full_period_str,comparative_full_period_str)
                print("Function 2- replace_cy_ya_with_actual_period END!")
                ...
                # last_level_values_list[1] += ' SALES'
                # last_level_values_list[2] += ' SALES'
                print('last_level_values_list=4806',last_level_values_list)
                # exit('exitt!')
                dict_selected_measures_lst = {}
                ...
                for key, indexes in facts_object_index_resp.items():
                    if 9 in indexes and 'Brand sales Index' not in filter_dict_resp:
                        # print(f'Key: {key}, Index mismatch error detected.')
                        facts_object_index_resp[key] = [-1 if x == 9 else x for x in indexes]
                        # print(f'Updated indexes: {facts_object_index_resp[key]}')
                ############# added on 19-02-2025 #######################################
                ...
                ###################################### ADDED ON 19-02-2025 ############################################
                period_replacement_map = {
                    'QUARTER': '',
                    'HY': ''
                }
     
                # Extract the period to be replaced from the string
                period_to_be_replaced_str = selected_full_period_str.split()[0]
     
                # Check if the period is in the replacement map
                if period_to_be_replaced_str in period_replacement_map:
                    replacement_value = period_replacement_map[period_to_be_replaced_str]
                   
                    if measure_row_column_position == "measure_in_column":
                        cross_df.rename(columns=lambda x: re.sub(period_to_be_replaced_str, replacement_value, x), inplace=True)
                    elif measure_row_column_position == "measure_in_row":
                        cross_df.rename(index=lambda x: re.sub(period_to_be_replaced_str, replacement_value, x), inplace=True)

                ######################## ADDED ON 19-02-2025 ####################################################
                if measure_row_column_position == "measure_in_row":
                    # Drop ('Grand Total', 'Grand Total') from rows if it exists
                    if ('Grand Total', 'Grand Total') in cross_df.index:
                        cross_df.drop(('Grand Total', 'Grand Total'), axis=0, inplace=True)
     
                    # Drop ('Grand Total', '') from columns if it exists
                    if ('Grand Total', '') in cross_df.columns:
                        cross_df.drop(('Grand Total', ''), axis=1, inplace=True)
     
                elif measure_row_column_position == "measure_in_column":
                    # Drop ('Grand Total', '') from rows if it exists
                    if ('Grand Total', '') in cross_df.index:
                        cross_df.drop(('Grand Total', ''), axis=0, inplace=True)
                    ...
                    if ('Grand Total', 'Grand Total') in cross_df.columns:
                        cross_df.drop(('Grand Total', 'Grand Total'), axis=1, inplace=True)
                ######################## ADDED ON 19-02-2025 ####################################################
                ...
                ############################################ ADDED ON 19-02-2025 ###################################
                # if any(var_name in (row_name, col_name) for var_name in [category_var_name, channel_var_name, brand_var_name]):
                #     print('var_name 5168-',var_name)
                #     print('category_var_name 5169-',category_var_name)
                #     print('channel_var_name 5170-',channel_var_name)
                #     print('brand_var_name 5171-',brand_var_name)
                #     try:
                #         cross_df = remove_prefix(cross_df)
                #     except Exception as e:
                #         pass  # Optionally log the exception if necessary
     
                # # Handle the renaming logic based on measure_row_column_position
                # try:
                #     if measure_row_column_position == "measure_in_column":
                #         cross_df.rename(columns=lambda x: re.sub('~}(Other panel)', '(Other panel)', x), inplace=True)
                #     elif measure_row_column_position == "measure_in_row":
                ...
                if asc_desc_param != 'no_sort':
                    try:
                        ...
                        pass
                #################################### ascending/descending order - 04-06-2024 #########
                print("Function 2- allign_grand_total_headers_fn STARTED!")
                # cross_df.to_excel('BEFORE_allign_grand_total_headers_fN.xlsx')
                if measure_row_column_position == "measure_in_column":
                    cross_df = allign_grand_total_headers_fn(cross_df)
                    ...
                    cross_df = cross_df.T
                try:    
                    cross_df = cross_df[~cross_df.index.isin([(slice(None, None, None), 'Grand Total')])]
                except:
                    pass
                print("Function 2- allign_grand_total_headers_fn END!")
                # cross_df.to_excel('crosss_dff_DERIVED.xlsx')
                # cross_df.to_excel(settings.FINAL_CROSSTAB_OUTPUT+'WRITE_CROSS_DF.xlsx')
                ######################## CROSSTAB LOGIC #####################################################
                ...
                responseValue = {
                        ...
                        "display_table":'Yes',
                        "filename_read":final_dest + filename,
...
def current_time_period_resp(request):
    if request.method == "POST":
        ...
        data = pd.read_csv(final_dest + filename, low_memory=False, engine='c')
...
def metrics_filter_call(request):
    if request.method == "POST":
        ...
        print("#############################################################")
        dict_base_filter_data_resp = request.POST.get('filter_data')
        # print('dict_base_filter_data_resp==',dict_base_filter_data_resp)
        dict_base_filter_data_resp_json = ast.literal_eval(dict_base_filter_data_resp)
        # dict_base_filter_data_resp_json = json.loads(dict_base_filter_data_resp_json11)
        ...
        print('selected_weight_column22 598',selected_weight_column22)
        for key in selected_weight_column22:
            dict_selected_measures_lst_NEW[key] = [item for item in last_level_values_list if item.startswith(key)]

        print('4134==dict_selected_measures_lst_NEW',dict_selected_measures_lst_NEW)
        ############### SELECTED - Metrics filter #################################

        ############### FILTERED - Metrics filter #################################
        print('facts_group_filter_index 5362--',facts_group_filter_index)

        for key, indexes in facts_group_filter_index.items():
            print('key2209:', key)
            print('indexes2210:', indexes)
            if (9 in indexes) and ('Brand sales index' not in facts_group_filter_index.keys()):
                print('2212 index mismatch error')
                indexes = [-1 if x == 9 else x for x in indexes]
                facts_group_filter_index[key] = indexes

        # Creating the filtered dictionary
        dict_selected_measures_filtered_lst  = {
            key: [dict_selected_measures_lst_NEW[key][i] for i in facts_group_filter_index[key]]
            for key in dict_selected_measures_lst_NEW
        }

        dict_selected_measures_filtered_lst = {key: list(set(value)) for key, value in dict_selected_measures_filtered_lst.items()
        }
        ...
        responseValue = {
                "status": 200,
                "comparative_time_period_resp":'time_period_dict',
                "metrics_all":dict_selected_measures_lst_NEW,
                "metrics_selected":dict_selected_measures_filtered_lst,
                "error": "Correct Data",
                # "merge_filename": filename_merged,
