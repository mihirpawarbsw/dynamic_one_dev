...
def crosstab_table(request):
    ...
    if request.method == "POST":
        ...
        ############ RENAME A VALUE - 15-05-2025 #################################
        if 'SKU' in filename: 
            df['Franchise'] = df['Franchise'].str.replace('24-Jul', '24/7', regex=False)

        df['Brand'] = df['Brand'].str.replace('Shiseido Global', 'Shiseido', regex=False)
        ############ RENAME A VALUE - 15-05-2025 #################################
