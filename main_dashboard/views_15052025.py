...
def crosstab_table(request):
    ...
    if request.method == "POST":
        ...
        df[categorical_cols] = df[categorical_cols].fillna("Undefined")
        ...
        # # Function to replace NaN with "Undefined"
        # def replace_nan(d):
        #     for key, value in d.items():
        #         if isinstance(value, dict):
        #             replace_nan(value)  # Recur for nested dictionaries
        #         elif isinstance(value, list):
        #             d[key] = [v if not pd.isna(v) else "Undefined" for v in value]
        #         elif pd.isna(value):
        #             d[key] = "Undefined"
        #     return d

        def replace_nan(d):
            # Predefine the "Undefined" value once to avoid redundant creation
            na_value = "Undefined"
        ...
        if (db_flag != 'Doors') or ((db_flag == 'Doors') and ('Door' not in selected_weight_column22)):
            ...
            df,selected_full_period_str,comparative_full_period_str = data_transformation_doors(filename,selected_weight_column22,selected_full_period,comparative_full_period,seperator_param,loop_vals_lst)
...
def crosstab_table_OLD_04022025(request):
    print("Function 1- crosstab_table STARED!")
    if request.method == "POST":
        measure_row_column_position = 'measure_in_column'
        agg_func = 'sum'
    ...
    if request.method == "POST":
        try:
            ...
            #####################################################################################
                ...
                #         pass
                # else:
                #     cross_df_UNFILTERED = pd.DataFrame()
                # # cross_df_UNFILTERED.to_excel('cross_df_UNFILTERED_MAIN.xlsx')
                # ################### ADDED ON 30-09-2024 - GRAND TOTAL ALL PIVOT #############
                # ################### ADDED ON 30-09-2024 - GRAND TOTAL ALL PIVOT #############
