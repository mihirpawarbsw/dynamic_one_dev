...
// var API_URL='https://bsstracker.brand-scapes.com:9000/';
// var API_URL='http://dynamicview.brand-scapes.com/';
...
function crosstable_filterdata(){
    ...
    var filter4=$("#filter4").val();
    var filter5=$("#filter5").val();
    var filter6=$("#filter6").val();
    var filter7=$("#filter7").val();
    var filter8=$("#filter8").val();
    var filter9=$("#filter9").val();
    var filter10=$("#filter10").val();
    var filter1_brandindex=$("#filter1_brandindex").val();
    var filter11=$("#filter1").attr("data-filtername");
    var filter22=$("#filter2").attr("data-filtername");
    var filter33=$("#filter3").attr("data-filtername");
    var filter44=$("#filter4").attr("data-filtername");
    var filter55=$("#filter5").attr("data-filtername");
    var filter66=$("#filter6").attr("data-filtername");
    var filter77=$("#filter7").attr("data-filtername");
    var filter88=$("#filter8").attr("data-filtername");
    var filter99=$("#filter9").attr("data-filtername");
    var filter100=$("#filter10").attr("data-filtername");
    var filter11_brandindex=$("#filter1_brandindex").attr("data-filtername");
    // filter_data.push({[filter11]:filter1,[filter22]:filter2,[filter33]:filter3,[filter44]:filter4,[filter55]:filter5});
  //   filter_data.push({[filter11]:filter1});
  // console.log('1111 filter2 function',typeof(filter1.length))
    if((filter1.length)!=0 && (filter2.length)!=0 && (filter3.length)!=0 && (filter4.length)!=0 && (filter5.length)!=0 && (filter6.length)!=0 && (filter7.length)!=0 && (filter8.length)!=0 && (filter9.length)!=0 && (filter100.length)!=0  ) {
         filter_data.push({[filter11]:filter1,[filter22]:filter2,[filter33]:filter3,[filter44]:filter4,[filter55]:filter5,[filter66]:filter6,[filter77]:filter7,[filter88]:filter8 ,[filter99]:filter9,[filter100]:filter10});
    }
    else if((filter1.length)!=0 && (filter2.length)!=0 && (filter3.length)!=0 && (filter4.length)!=0 && (filter5.length)!=0 && (filter6.length)!=0 && (filter7.length)!=0 && (filter8.length)!=0 && (filter9.length)!=0  ) {
         filter_data.push({[filter11]:filter1,[filter22]:filter2,[filter33]:filter3,[filter44]:filter4,[filter55]:filter5,[filter66]:filter6,[filter77]:filter7,[filter88]:filter8 ,[filter99]:filter9});
    }
    else if((filter1.length)!=0 && (filter2.length)!=0 && (filter3.length)!=0 && (filter4.length)!=0 && (filter5.length)!=0 && (filter6.length)!=0 && (filter7.length)!=0 && (filter8.length)!=0  ) {
         filter_data.push({[filter11]:filter1,[filter22]:filter2,[filter33]:filter3,[filter44]:filter4,[filter55]:filter5,[filter66]:filter6,[filter77]:filter7,[filter88]:filter8});
    }


    else if((filter1.length)!=0 && (filter2.length)!=0 && (filter3.length)!=0 && (filter4.length)!=0 && (filter5.length)!=0 && (filter6.length)!=0 && (filter7.length)!=0  ) {
         filter_data.push({[filter11]:filter1,[filter22]:filter2,[filter33]:filter3,[filter44]:filter4,[filter55]:filter5,[filter66]:filter6,[filter77]:filter7});
    }

    else if((filter1.length)!=0 && (filter2.length)!=0 && (filter3.length)!=0 && (filter4.length)!=0 && (filter5.length)!=0 && (filter6.length)!=0) {
         filter_data.push({[filter11]:filter1,[filter22]:filter2,[filter33]:filter3,[filter44]:filter4,[filter55]:filter5,[filter66]:filter6});
    }
    else if((filter1.length)!=0 && (filter2.length)!=0 && (filter3.length)!=0 && (filter4.length)!=0 && (filter5.length!=0)) {
         filter_data.push({[filter11]:filter1,[filter22]:filter2,[filter33]:filter3,[filter44]:filter4,[filter55]:filter5});
    }
    ...
    if((filter1_brandindex.length)!=0) {
         filter_data.push({[filter11_brandindex]:filter1_brandindex});
    }
    ...
    // console.log('group_column_resp_keyname--->985',resp['df_cross_json']['columns'][0].length)
        ...
        $.each(group_column_resp_result, function (k2, val2) {
            ...
            $.each(val2, function (k3, val3) {
                ...
                html2 +='<th scope="col"  class="align-baseline text-lg-center 3"  colspan='+val3_count+' style="background-color: #595959 !important;;color:white">'+result3+' '+sort_arrow_code+'</th>';
                ...
                if (column_length==3) {
                    ...
                    $.each(val3, function (k4, val4) {
                        ...
                        if (val4.indexOf("Rank") !== -1) {
                            ...
                            // console.log("The substring 'Rank' does not exist in the string.");
                            var sort_arrow_code='<span class="pl-4 column_level_'+header_i+'" id="column_level_'+header_i+'" data-sortflag="desc_sort" data-column1="'+sort_flag_array+'" onclick="table_sorting_function(this)"><i class="fa fa-sort" aria-hidden="true" style="font-size:12px; cursor:pointer; position:relative; left:7px; color:white;"></i></span>';
                        }
