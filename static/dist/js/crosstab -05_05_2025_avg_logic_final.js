...
function display_crosstab_table(resp,calculation_type_name,facts_selected_value){
  ...
  var copy_variable=total_column_length;
    ...
    console.log('get_metrics_val-->1111',get_metrics_val)
    console.log('get_metrics_val_len-->',get_metrics_val_len)
    ...
    get_metrics_val.forEach(item => {
      const doorMatch = item.match(/^Door_QUARTER (Q[1-4] \d{4})$/);

      if (doorMatch) {
        const quarterYear = doorMatch[1];

        // Match: any "Sales..._QUARTER Qx YYYY"
        const salesItem = get_metrics_val.find(s =>
          s.startsWith("Sales") && s.includes(`QUARTER ${quarterYear}`)
        );

        if (salesItem) {
          find_absolute_metrice_dynamic_for_match.push([item, salesItem]);
        }
        // Do nothing if no match found (don't push empty array)
      }
    });
    ...
    find_absolute_metrice_dynamic_for_match.forEach((item, index) => {
      keyedOutput_absolute_metrice[`Avg${index + 1}`] = item;
    });
    ...
    console.log('keyedOutput_absolute_metrice_final -->1111',keyedOutput_absolute_metrice_final);
    ...
    console.log('find_absolute_metrice_dynamic_for_match---->1111',find_absolute_metrice_dynamic_for_match);
    console.log('keyedOutput_absolute_metrice---->1111',keyedOutput_absolute_metrice);
    console.log('find_absolute_metrice_dynamic_for_match length---->1111',nonEmptyCount);
    ...
    if (rowfilter_sort.length==1 && columnfilter_sort.length==0 && measure_type=='measure_in_column' && metrics_selected_Values_length==2 && check_door_sales_key_present=='yes' && nonEmptyCount>=1) {
            ...
            if (Object.keys(keyedOutput_absolute_metrice).length !== 0) {
              ...
              for (const key in group_column_resp) {
                if (key === "Grand Total") {
                  group_column_resp_1["Average"] = averageData;
                }
            ...
            console.log('####################################################')
            // console.log('html-800',html)
            console.log('group_column_resp-1111',group_column_resp_1)
            // console.log('single_row_length-800',single_row_length)
            // console.log('row_str-800',row_str)
            console.log('columns_data-1111',columns_data)
            // console.log('resp-800',resp)
            console.log('####################################################')
                        ...
                        // $.each(resp['df_cross_json']['data'][i], function (k1, val2) {
                            ...
                            if (k1 === calculation_extra_row.length - get_metrics_val_len && nonEmptyCount>0) {
                                ...
                                $.each(average_door_values, function (avg_key, avg_value) {
                                  html += '<th scope="row" class="text-right">' + addThousandSeparators(avg_value) + '</th>';
                                });
