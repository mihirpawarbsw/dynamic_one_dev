...
selected_filename = selected_filename1.replace('Marketwise_', ''); // Remove 'Marketwise_' if present
selected_filename = selected_filename.replace('Consolidated_', ''); // Remove 'Consolidated_' including the trailing underscore
...
// This condition work when user click on view history on first load start here
    ...
    if (verify_resp['code']==200) {
            ...
            filters_assign_fun(respdata4['filter_dict_resp'],filters_data_selected);
            // #################################################################
...
$('#comparison_time_period,#currency_type').change(function (e){
            ...
            $.each(value, function (key2, value1) {
                ...
                // var  {replace_string,original_string} = replace_CY_YA_string(value1,current_time_period_resp,comparative_time_period_resp);
                    ...
                    if (found) {
                        ...
                        $("#facts_group_filter #"+key1.replace(/[^a-zA-Z0-9]/g, '') ).append('<option selected class="dropmenu" value="'+value1+'" >'+result.replace("QUARTER", "")+'</option>');
                    }
                    else{
                        $("#facts_group_filter #"+key1.replace(/[^a-zA-Z0-9]/g, '') ).append('<option class="dropmenu" value="'+value1+'" >'+result.replace("QUARTER", "")+'</option>');
                    }
            });
...
function filters_assign_fun(filter_dict_resp,filters_data_selected){
    ...
    $.each(filter_dict_resp, function (key1, value1) {
        ...
        if (Array.isArray(filters_data_selected) && filters_data_selected.length > 0) {
            selected_val = filters_data_selected[0][key1];
        }
        ...
        console.log('end ######################################')
          ...
          // Reordering the array
            ...
            if (key1=='Brand') {
                ...
                // console.log('rowfilter_sort_lenght 600',rowfilter_sort_lenght,typeof(rowfilter_sort_lenght))
                // console.log('measure_type_value 600',measure_type_value,typeof(measure_type_value))
                // console.log('rowfilter_sort 600',rowfilter_sort[0],typeof(rowfilter_sort[0]))
                if (rowfilter_sort_lenght===1 && measure_type_value==="Measure in Row" && rowfilter_sort[0]==='Brand') {
                        console.log('line 9000')
                ...
                // console.log('####################### start ##################')
                    $.each(resp['df_cross_json']['data'][counter_i], function (k1, val2) {
                        ...
                        // $.each(resp['df_cross_json']['data'][i], function (k1, val2) {
                            ...
                            if (grand_total_index_array_json[k1]['L2']=='Grand Total' || row_k1=='Grand Total' || row_k=='Grand Total') {
                            ...
                            if (Object.keys(check_stored_object_json).length !== 0) {
                                ...
                                if (typeof(matchingValues1) !== 'undefined' && matchingValues1 !== null) {
                                    ...
                                    html +='<th scope="row" class="text-right" style="color:'+get_color[1]+';background-color:'+grand_total_bg+'" >'+addThousandSeparators(val_text)+'</th>';
                                }
                                else{
                                    // console.log('if else condition 1')
                                    html +='<th scope="row" class="text-right" style="color:'+find_color_code[0]+';background-color:'+grand_total_bg+'">'+addThousandSeparators(val_text)+'</th>';
                                }
  ...
  var counter_i=0;
    $.each(group_row_resp_result, function (row_k, row_val) {
        ...
        $.each(row_val, function (row_k1, row_val1) {
            ...
            if (numberOfLevels===2) {
                var row_val1_count=Object.keys(row_val1).length;
                html +='<th scope="row" class="bg-info" rowspan="'+row_val1_count+'" style="background-color:#e0e1dd !important">'+row_k1+'</th>';

                $.each(row_val1, function (row_k2, row_val2) {
                    let result1 = row_k2.split('_')[1].trim();
                    html +='<th scope="row" class="bg-info L3 row_l3"  style="background-color:#e0e1dd !important">'+row_k2+'</th>';
                    // console.log('start ####################################################')
                    // console.log('row_k1==>',row_k1)
                    // console.log('Measures==>',row_k2)
            ...
            if (filteredData1.length!=0) {
                ...
                $.each(filteredData1, function (key1, value2) {
                    ...
                    if (get_facts_colorizer_object!="") {
                       ...
                       // console.log('selected_facts_name_1 4000',selected_facts_name_1);
                       ...
                       // console.log('get_result factsvalue 4000',get_result.factsvalue);
                        ...
                        html+='<input type="checkbox" class="form-check-input" id="checkbox_'+joinedText+'_'+tp_key+'_'+key1+'" data-tp_name="'+tp_value+'"  data-factsheader="'+facts_key+'" "'+result+'" disabled readonly>';
