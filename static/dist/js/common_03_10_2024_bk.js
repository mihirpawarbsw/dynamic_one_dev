...
function filter_checkdata(rowfilter_sort,columnfilter_sort){
    ...
    datafilter_check.forEach(item => {
        ...
        if (combined_row_column.includes(item.filter)) {
            ...
            if (item.value.length > 0) {
                // Do something if the value array has elements
            } else {
                console.log(`${item.filter} has an empty value array.`);
                // alert(`${item.filter} has an empty value array.`);
                
                // Stop the loading overlay
                $.LoadingOverlay("hide", true);
                
                // Throw an error to stop the loop and script execution
                throw new Error(`${item.filter} has an empty value array.`);
            }
        }
    });
} catch (error) {
    alert(error.message,`has an empty value array.11`);
    // console.error(error.message); // Handle the error, log the message
}
