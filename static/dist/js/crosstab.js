...
// This condition work when user click on view history on first load start here
    ...
    if (verify_resp['code']==200) {
            ...
            var market_total =resps_history['data'][0]['market_total'];
            
            var comparative_time_period =resps_history['data'][0]['comparative_time_period'];
            console.log('comparative_time_period 259',comparative_time_period)
            ...
            Total_column_filter=$('input[name="Total_column"]:checked').val();
             // alert(market_total)
            if (market_total=='gt_all') {

                document.getElementById("display_grand_total_flag").checked = true;
            }
            // else{
            //     document.getElementById("display_grand_total_flag").checked = false;
            // }
...
$('#Add_to_favourite').on('click', function() {
    ...
    if (add_to_favourite_name!='' || add_to_favourite_name==undefined) {
        var checkData = {
            ...
            'display_grand_total_flag': display_grand_total_flag,
...
function Facts_chart(resp){
 ...
 function facts_assign_fun(resp,selected_resp1,current_time_period_resp,comparative_time_period_resp) {
    ...
    console.log('rowfilter_sort-->999',rowfilter_sort)
    console.log('colfilter_sort-->999',columnfilter_sort)
    // Average sale per door logic start
    // if(rowfilter_sort.length==1 && columnfilter_sort.length==0){
    //     if (rowfilter_sort[0]=='Brand') {
    //         resp['Average'] = ['Average Sale per door'];
    //         selected_resp1['Average'] = ['Average'];

    //     }
    // }
    // console.log('after facts_assign_fun==328',selected_resp1);
    // console.log('after resp==328',resp);
    // Average sale per door logic end
    var i=
        $.each(resp, function (key1, value) {
                $("#facts_group_filter").append('<optgroup label="'+key1+'" id="'+key1.replace(/[^a-zA-Z0-9]/g, '')+'" class="dropmenu">');
            $.each(value, function (key2, value1) {
                // console.log('facts_assign_fun inner==328',value1)
                ...
                let result = (typeof value1 === 'string' && value1.includes('_') && value1.split('_')[1]) ? value1.split('_')[1].trim() : value1;
          ...
          // Reordering the array
            ...
            if (key1=='Brand') {
                ...
                // if (rowfilter_sort_lenght===1 && measure_type_value==="Measure in Row" && rowfilter_sort[0]==='Brand') {
                    ...
                    // console.log('========== rowfilter_sort_lenght',rowfilter_sort_lenght)
                    // console.log('========== measure_type_value',measure_type_value)
...
function checkDoorAndSalesKeys(data) {
  try {
    const keys = Object.keys(data).map(k => k.toLowerCase());
    const hasDoor = keys.includes("door") && Array.isArray(data["Door"]) && data["Door"].length > 0;

    const salesKey = Object.keys(data).find(k => k.toLowerCase().includes("sales"));
    const hasSales = salesKey && Array.isArray(data[salesKey]) && data[salesKey].length > 0;

    return hasDoor && hasSales ? "yes" : "no";
  } catch (err) {
    return "no";
  }
}
...
function display_crosstab_table(resp,calculation_type_name,facts_selected_value){
    ...
    // console.log('row_data 1882',row_data)
  ...
  var copy_variable=total_column_length;
    ...
    // ############################################################################################
    //NEW LOGIC FOR AVERAGE SALES DOOR END  30-04-2025 HERE

    const get_metrics_val=$("#facts_group_filter").val();
    const get_metrics_val_len=get_metrics_val.length;
    // console.log('get_metrics_val-->5000',get_metrics_val)
    // console.log('get_metrics_val_len-->5000',get_metrics_val_len)
    ...
    var columnfilter_sort=exract_one_array_object_by_labels(c_text_val);
    var measure_type=$('#hiddevalue').text();
    var metrics_selected_Values = getSelectedValues_facts_filter();
    let metrics_selected_Values_length = Object.keys(metrics_selected_Values).length;
    var check_door_sales_key_present=checkDoorAndSalesKeys(metrics_selected_Values);
    // console.log('r_text_val-->2222',r_text_val)
    // console.log('c_text_val-->2222',c_text_val)
    // console.log('rowfilter_sort-->2222',rowfilter_sort)
    // console.log('columnfilter_sort-->2222',columnfilter_sort)
    // console.log('measure_type-->2222',measure_type)
    // console.log('metrics_selected_Values-->2222',metrics_selected_Values)
    // console.log('metrics_selected_Values length-->2222',metrics_selected_Values_length)
    // console.log('check door and sales 2222',checkDoorAndSalesKeys(metrics_selected_Values));

    // if (rowfilter_sort.length==1 && columnfilter_sort.length==0) {
    //     if (rowfilter_sort[0]=='Brand' && measure_type=='measure_in_column' && metrics_selected_Values_length==2 && check_door_sales_key_present=='yes') {
    //         alert('yes')
    //     }
    // }

    // checking the metric absolute  combination array start
    const find_absolute_metrice_dynamic_for_match = [];

   

   // WORKING CODE BASE ON LOGIC OLD CODE START
    
    get_metrics_val.forEach(item => {
      const cleanedItem = item.trim();

      // Match formats like: Door_FY 2024 Door, Door_HY H1 2024 Door, Door_QUARTER Q4 2024 Door
      const match = cleanedItem.match(/^Door_(QUARTER|YTD|MAT|HY|FY)[ _]?(Q[1-4]|H[1-2])?\s?(\d{4})/i);
      console.log('match  -->5000 ',match)
      if (match) {
        console.log('match if -->5000 before',match)
        const type = match[1].toUpperCase();         // FY, HY, etc.
        const periodPart = match[2] ? match[2].toUpperCase() + ' ' : ''; // Q4 or H2 if exists
        const year = match[3];                       // 2024
        const fullPeriod = `${type} ${periodPart}${year}`.trim();

        const salesItem = get_metrics_val.find(s =>
          s.startsWith("Sales") &&
          s.includes(fullPeriod) &&
          !/share|Share|GR|%|rank/i.test(s)
        );

        if (salesItem) {
            console.log('salesItem if -->5000 before',salesItem)
          find_absolute_metrice_dynamic_for_match.push([cleanedItem, salesItem]);
        }
      }
    });
    ...
    console.log('find_absolute_metrice_dynamic_for_match-->5000 before',find_absolute_metrice_dynamic_for_match)
    // WORKING CODE BASE ON LOGIC NEW CODE END


     // checking the metric absolute  combination array end

     // Removing the QUARTER, YTD, MAT, HY, FY from array elements
    //Removing the QUARTER from array for match column index key
    find_absolute_metrice_dynamic_for_match.forEach((subArray, i) => {
      subArray.forEach((str, j) => {
        find_absolute_metrice_dynamic_for_match[i][j] = str.replace("QUARTER", "").replace(/\s{2,}/g, ' ');
      });
    });
    find_absolute_metrice_dynamic_for_match.forEach((subArray, i) => {
      subArray.forEach((str, j) => {
        find_absolute_metrice_dynamic_for_match[i][j] = str.replace("HY", "").replace(/\s{2,}/g, ' ');
      });
    });
  
    console.log('find_absolute_metrice_dynamic_for_match-->6666 after',find_absolute_metrice_dynamic_for_match)

    // adding  key in absolute  combination array start
  
   
    const keyedOutput_absolute_metrice = {};

    find_absolute_metrice_dynamic_for_match.forEach((item, index) => {
      const str = item[0];

      const time_range = $("#time_range").val(); // fallback

      // Main match: Type + optional Qx/Hx + Year
      const fullMatch = str.match(/\b(FY|YTD|MAT|HY|QUARTER)\s*(Q[1-4]|H[1-2])?\s*(\d{4})\b/i);

      if (fullMatch) {
        const type = fullMatch[1] ? fullMatch[1].toUpperCase() : time_range;
        const period = fullMatch[2] ? fullMatch[2].toUpperCase() + " " : "";
        const year = fullMatch[3] || "Unknown";

        const key = `Average ${type}_ ${period}${year}`;
        keyedOutput_absolute_metrice[key] = item;
      } else {
        // Fallback: match H1/H2 or Q1–Q4 with year, even if type missing
        const fallbackMatch = str.match(/\b(H[1-2]|Q[1-4])\s+(\d{4})\b/i);

        if (fallbackMatch) {
          const period = fallbackMatch[1].toUpperCase();
          const year = fallbackMatch[2];
          const key = `Average _ ${period} ${year}`;
          keyedOutput_absolute_metrice[key] = item;
        } else {
          // Final fallback: just use year if present
          const yearOnlyMatch = str.match(/\b(\d{4})\b/);
          const year = yearOnlyMatch ? yearOnlyMatch[1] : `Unknown_${index + 1}`;
          const key = ` Average _ ${year}`;
          keyedOutput_absolute_metrice[key] = item;
        }
      }
    });
    ...
    //adding index of match absolute value start
    const keyedOutput_absolute_metrice_final = {};

    for (const key in keyedOutput_absolute_metrice) {
      const valueArray = keyedOutput_absolute_metrice[key];
      const indexArray = valueArray.map(val => {
        const idx = columns_data.findIndex(col => col[1] === val);
        return idx !== -1 ? idx : null; // or handle null if not found
      });
      keyedOutput_absolute_metrice_final[key] = indexArray;
    }
    
    // console.log('keyedOutput_absolute_metrice-->6666',keyedOutput_absolute_metrice)
    // console.log('columns_data-->6666',columns_data) 
    // console.log('keyedOutput_absolute_metrice_final -->6666',keyedOutput_absolute_metrice_final);
    //adding index of match absolute value end



    const nonEmptyCount = find_absolute_metrice_dynamic_for_match.filter(innerArray => innerArray.length > 0).length;
    // console.log('find_absolute_metrice_dynamic_for_match---->444',find_absolute_metrice_dynamic_for_match);
    // console.log('keyedOutput_absolute_metrice---->444',keyedOutput_absolute_metrice);
    // console.log('find_absolute_metrice_dynamic_for_match length---->444',nonEmptyCount);
     // rowfilter_sort[0]=='Brand'
    if (rowfilter_sort.length==1 && columnfilter_sort.length==0 && measure_type=='measure_in_column' && metrics_selected_Values_length==2 && check_door_sales_key_present=='yes' && nonEmptyCount>=1) {
            // alert('yes')

            // #######################################################################################
            // average metric combination append in array start

            //############################################################
            const group_column_resp_1 = {};
            if (Object.keys(keyedOutput_absolute_metrice).length !== 0) {
              const averageData = [];

              // Step 1: Generate Average Data from output keys
              for (const key in keyedOutput_absolute_metrice) {
                averageData.push(["Average Sales per door", key]);
              }

              // Step 2: Merge into group_column_resp_1
              
              for (const key in group_column_resp) {
                if (key === "Grand Total") {
                  group_column_resp_1["Average Sales per door"] = averageData;
                }
                group_column_resp_1[key] = group_column_resp[key];
              }

              // Step 3: Insert into columns_data before "Grand Total"
              const index = columns_data.findIndex(item => item[0] === "Grand Total");
              if (index !== -1) {
                // Insert in reverse to preserve order
                for (let i = averageData.length - 1; i >= 0; i--) {
                  columns_data.splice(index, 0, averageData[i]);
                }
              }
            }
            ...
            // console.log('####################################################')
            // console.log('group_column_resp-1111',group_column_resp_1)
            // console.log('columns_data-1111',columns_data)
            // console.log('####################################################')


            html= header_stacked_logic(html,group_column_resp_1,single_row_length,row_str,columns_data,resp);

            // average metric combination append in array end
             // #######################################################################################
         //NEW LOGIC FOR AVERAGE SALES DOOR END  30-04-2025 HERE
    // ############################################################################################
    }else{
        html= header_stacked_logic(html,group_column_resp,single_row_length,row_str,columns_data,resp);//old code
    ...
    // console.log('group_column_resp_1-800',group_column_resp_1)
    // html= header_stacked_logic(html,group_column_resp,single_row_length,row_str,columns_data,resp);//old code
// header cloumn name end here
  ...
  if (get_facts_colorizer_object != "") {
    ...
    console.log('grand_total_index_array_object-->8000',grand_total_index_array_json)
    // console.log('decimal_point_filter-->400',decimal_point_filter)
    // console.log('group_row_resp_result-->400',group_row_resp_result)
  var counter_i=0;
        ...
        $.each(row_val, function (row_k1, row_val1) {
            ...
            if (numberOfLevels===1) {
                html +='<th scope="row" class="bg-info L2 right-scroller">'+row_k1+'</th>';
                    ...
                    //  }
                    // console.log('################## tr open #################')
                    // console.log(' data check -->1',resp['df_cross_json']['data'])
                    // console.log(' data check -->2',resp['df_cross_json']['data'][counter_i])
                    const calculation_extra_row=resp['df_cross_json']['data'][counter_i];
                    // const door1 = parseFloat(calculation_extra_row[0], 10);
                    // const door2 = parseFloat(calculation_extra_row[0], 10);
                    // const sales1 = parseFloat(calculation_extra_row[2], 10);
                    // const sales2 = parseFloat(calculation_extra_row[2], 10);
                    // // Safe division with fallback to 'N/A' if denominator is 0
                    // const t1 = door1 !== 0 ? ((sales1 / door1)*100).toFixed(parseInt(decimal_point_filter)) : 'N/A';
                    // const t2 = door2 !== 0 ? ((sales1 / door1)*100).toFixed(parseInt(decimal_point_filter)) : 'N/A';

                    if (rowfilter_sort.length==1 && columnfilter_sort.length==0  && measure_type=='measure_in_column' && metrics_selected_Values_length==2 && check_door_sales_key_present=='yes' && nonEmptyCount>=1) {
                    // #######################################################################
                    // new logic start for AVG

                    // var average_door_values = {}; // Store t1, t2... dynamically

                    // let t_index = 1;
                    // for (var key in keyedOutput_absolute_metrice_final) {
                    //   var indices = keyedOutput_absolute_metrice_final[key];
                    //   var doorVal = parseFloat(calculation_extra_row[indices[0]]);
                    //   var salesVal = parseFloat(calculation_extra_row[indices[1]]);

                    //   var tVal = doorVal !== 0 ? (salesVal / doorVal).toFixed(parseInt(decimal_point_filter)) : 0;

                    //   // Dynamically assign t1, t2, t3...
                    //   average_door_values[`t${t_index}`] = tVal;
                    //   t_index++;
                    // }


                    var average_door_values = {}; // Store t1, t2, ...
                    let t_index = 1;
                    // Loop through keyedOutput_absolute_metrice_final
                    for (const key in keyedOutput_absolute_metrice_final) {
                      const indices = keyedOutput_absolute_metrice_final[key];

                      if (!indices || indices.length < 2) {
                        average_door_values[`t${t_index}`] = 'N/A';
                        t_index++;
                        continue;
                      }

                      // Extract values safely
                      let doorVal = parseFloat(calculation_extra_row[indices[0]]);
                      let salesVal = parseFloat(calculation_extra_row[indices[1]]);

                      // Convert NaN or strings like "0.0" properly
                      doorVal = isNaN(doorVal) ? 0 : doorVal;
                      salesVal = isNaN(salesVal) ? 0 : salesVal;

                      // Safe division
                      const tVal = doorVal !== 0 ? (salesVal / doorVal).toFixed(parseInt(decimal_point_filter)) : 'N/A';
                    ...
                    console.log('################## AVG start 8888');
                    console.log('keyedOutput_absolute_metrice_final 8888',keyedOutput_absolute_metrice_final);
                    console.log('average_door_values 8888',average_door_values);
                    console.log('calculation_extra_row 8888',calculation_extra_row);
                    console.log('################## AVG start 8888');

                    // if (k1 === calculation_extra_row.length - get_metrics_val_len && nonEmptyCount>0) {
                    if (nonEmptyCount>0) {
                        $.each(average_door_values, function (avg_key, avg_value) {
                          // Insert at the position before the last 4 elements
                          var insertIndex = resp['df_cross_json']['data'][counter_i].length - get_metrics_val_len;
                          resp['df_cross_json']['data'][counter_i].splice(insertIndex, 0, avg_value);
                        });
                    }
                    // new logic start for AVG END
                    // #######################################################################
                 ...
                 console.log('final response 6666-->',resp['df_cross_json']['data'][counter_i])
                console.log('####################### start ################## ->7777')
                    $.each(resp['df_cross_json']['data'][counter_i], function (k1, val2) {
                        // console.log('############### start loop ######################')
                        // console.log(' T1 calculation--> door1',door1,'sales1',sales1,'sales2',sales2)
                    // $.each(resp['df_cross_json']['data'][counter_i], function (k1, val2_val) {
                       ...
                       //  const varPercent = val2_val.toString().includes('%') ? '%' : ' ';
                        ...
                        // $.each(resp['df_cross_json']['data'][i], function (k1, val2) {
                            ...
                            console.log('grand_total_index_array_json__777',grand_total_index_array_json[k1]['L2'])
                            console.log('k1 777',k1)
                            console.log('val2 777',val2)
                            // console.log('row_k 777',row_k)
                            ...
                            // adding default color code for GR and BR logic end
                             ...
                             // Before the last column, insert T1 and T2
                            // if (k1 === calculation_extra_row.length - get_metrics_val_len && nonEmptyCount>0) {
                            //     // html +='<th scope="row" class="text-right">'+addThousandSeparators(t1)+'</th>';
                            //     // html +='<th scope="row" class="text-right">'+addThousandSeparators(t2)+'</th>';
                            //     $.each(average_door_values, function (avg_key, avg_value) {
                            //       html += '<th scope="row" class="text-right AVERAGE" >' + addThousandSeparators(avg_value) + '</th>';
                            //       // var grand_total_bg='#f3f2ec';
                            //     });
                            ...
                            if (Object.keys(check_stored_object_json).length !== 0) {
                                ...
                                console.log('if else condition 1-->9999')
                            }
                            else{

                                console.log('if condition 2-->9999')
                                ...
                                // console.log('data key-->',k1,'value text--->',val_text,'value type--->',typeof(val_text))
                    ...
                    // console.log('################## tr closed #############')
                    console.log('####################### END ##################-->7777')
                    counter_i++;
            }
...
//          $.LoadingOverlay("hide",true);
// });
function crosstable_filterdata_all() {
    var filter_data = [];

    // Loop through all filters dynamically
    $("[id^='filter']").each(function () {
        var id = $(this).attr("id");
        var allValues = [];

        // Fetch all possible options (both selected & unselected)
        ...
        if (filterName) {
            filter_data.push({ [filterName]: allValues }); // Store values in array format
        }
    });
...
$("#filter_submit").click(function(){
    ...
    setTimeout(function () {
        // filter_data=crosstable_filterdata();
        filter_data=crosstable_filterdata_all();
        console.log('filter_data-->500',filter_data)
        // console.log('filter_data_all-->500',filter_data_all)
    ...
    var checkData4 = "csrfmiddlewaretoken=" + csrfmiddlewaretoken+"&rowfilter_val=" + encodeURIComponent(JSON.stringify(rowfilter_val))+"&columnfilter_val=" + encodeURIComponent(JSON.stringify(columnfilter_val))+"&calculation_type_name=" + calculation_type_name+"&weight_type_name=" + weight_type_name+"&tbl_name=" + tbl_name+"&seperated_flag_row_2=" + seperated_flag_row_2+"&seperated_flag_col_2=" + seperated_flag_col_2+"&weight_volume_type_name=" + weight_volume_type_name+"&final_row_col_array_grp=" + encodeURIComponent(JSON.stringify(final_row_col_array_grp2))+"&filename_merged=" + table_name+"&table_data_type_respone=" + table_data_type_respone+"&decimal_point_filter=" + parseInt(decimal_point_filter)+"&Total_column_filter=" + Total_column_filter+"&filter_data='" + encodeURIComponent(filter_data_new)+"'&base_column=" + base_column+"&wt_measures=" + JSON.stringify(wt_measures)+"&Time_val=" + time_val+"&measure_type=" + measure_type+"&facts_object=" + JSON.stringify(getSelectedValues_facts)+"&facts_object_index=" + JSON.stringify(getSelectedValues_facts_all)+"&comparative_time_period=" + comparative_time_period+"&column_name_sort_table=" + encodeURIComponent(JSON.stringify(column_name_sort_table))+"&table_sort_flag=" + table_sort_flag+"&currency_type=" + currency_type+"&display_grand_total_flag=" + display_grand_total_flag;
        ...
        respdata4 = autoresponse("crosstab_table_page2", checkData4);
                ...
                if (respdata4.status  == 200)
                    ...
                    // console.log('rowfilter_sort 500 type',typeof(columnfilter_sort.length))
                    // console.log('columnfilter_sort length 500',columnfilter_sort.length)
                   ...
                   if ((rowfilter_sort.length == 1 && columnfilter_sort.length == 1) || (rowfilter_sort.length == 1 && columnfilter_sort.length == 0)) {
                    ...
                    else if(rowfilter_sort.length==1 && rowfilter_sort[0]=='Brand' && measure_type_value=='Measure in Row'){
                        // alert('else if new')
                        // console.log('measure_type_value 404-->',measure_type_value)
                        $('#filter_div_brandindex').removeClass('d-none').addClass('d-block');
                    }
                    else{
                        // alert('else 2')
...
function submit_colorizer_data() {
    ...
    for (var i =0; i<card_len; i++) {
        ...
        $("#collapse"+i+" .card-body .d-flex").each(function(e){
            ...
            console.log('input1---> 4300 line',input1)
            console.log('input2---> 4300 line',input2)
            console.log('input1_ck---> 4300 line',input1_ck)
            console.log('input2_ck---> 4300 line',input2_ck)
