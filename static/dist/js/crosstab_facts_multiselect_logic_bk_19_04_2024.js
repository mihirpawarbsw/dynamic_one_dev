...
var final_row_col_array_grp2 = JSON.parse(final_row_col_array_grp1);
	...
	setTimeout(function () { 
			...
			var checkData5={'csrfmiddlewaretoken': csrfmiddlewaretoken,'filename':filename[0]};		
   ...
   setTimeout(function () {
			...
			display_crosstab_table(respdata4,calculation_type_name);
			display_crosstab_table_xls_download(respdata4,calculation_type_name);
...
function time_period_filters(resp){
...
function comparative_time_period_resp() {
	...
	var checkData5={'csrfmiddlewaretoken': csrfmiddlewaretoken,'current_timeperiod':current_timeperiod,'filename':filename[0]};		
	resp_filter = autoresponse("comparative_time_period_resp", checkData5);
	console.log('comparative_time_period_resp',resp_filter)
	if (resp_filter['status']==200) {
		// var tp_active=$("time_period").val();

		var tp_active=$("#time_period").val();
		// Extract the last 4 digits using a regular expression
		var lastDigits = tp_active.match(/\d{4}$/);
		// Convert the extracted digits to a number and subtract 1
		var newYear = parseInt(lastDigits[0]) - 1;
		// Replace the last 4 digits in the string with the new year
		var newString = tp_active.replace(/\d{4}$/, newYear);
		console.log('newString------->',newString)

		$.each(resp_filter['comparative_time_period_resp'], function (key1, value) {


 				$("#comparison_time_period").append('<optgroup label="'+key1+'" id="'+key1.replace(/[^a-zA-Z0-9]/g, '')+'" class="dropmenu">');
			$.each(value, function (key2, value2) {
				...
				var value2_original=value2;
			 	if (value2.includes("QUARTER")) {
				    value2 = value2.replace("QUARTER", "");
				    if (value2==newString.replace("QUARTER", "")) {
			 			$("#comparison_time_period").append('<option style="width: 100px" selected  value="'+value2_original+'" >'+value2+'</option>');
				 	}else{
				 		$("#comparison_time_period").append('<option  style="width: 100px" value="'+value2_original+'" >'+value2+'</option>');
				 	}
				}else{
					if (value2==newString.replace("QUARTER", "")) {
			 			$("#comparison_time_period").append('<option style="width: 100px" selected  value="'+value2+'" >'+value2+'</option>');
				 	}else{
				 		$("#comparison_time_period").append('<option  style="width: 100px" value="'+value2+'" >'+value2+'</option>');
				 	}
				}


				// $("#comparison_time_period").append('<option class="dropmenu"value="'+value1+'" >'+value1+'</option>');
			...
			$.each(value, function (key2, value1) {
				...
				// var  {replace_string,original_string} = replace_CY_YA_string(value1,current_time_period_resp,comparative_time_period_resp);
					...
					if (found) {
						...
						$("#facts_group_filter #"+key1.replace(/[^a-zA-Z0-9]/g, '') ).append('<option selected class="dropmenu" value="'+value1+'" >'+value1+'</option>');
					}
					else{
						$("#facts_group_filter #"+key1.replace(/[^a-zA-Z0-9]/g, '') ).append('<option class="dropmenu" value="'+value1+'" >'+value1+'</option>');
					}
			});
...
function display_crosstab_table(resp,calculation_type_name){
...
// header cloumn name end here
  ...
  var counter_i=0;
	$.each(group_row_resp_result, function (row_k, row_val) {
		...
		html +='<th scope="row" class="bg-info L1"  rowspan='+row_val_len+'>'+row_k+'</th>';
		...
		$.each(row_val, function (row_k1, row_val1) {
			...
			if (numberOfLevels===2) {
				...
				$.each(row_val1, function (row_k2, row_val2) {
					...
					html +='<th scope="row" class="bg-info L3">'+row_k2+'</th>';
					// console.log('start ####################################################')
...
function display_crosstab_table_xls_download(resp,calculation_type_name){
		...
		$.each(row_val, function (row_k1, row_val1) {
			...
			if (numberOfLevels===2) {
				...
				$.each(row_val1, function (row_k2, row_val2) {
					...
					html +='<th scope="row" class="bg-info L3">'+hasMatchingSubstring(row_k2)+'</th>';
					// console.log('start ####################################################')
					// console.log('row_k1==>',row_k1)
					// console.log('Measures==>',row_k2)
					$.each(resp['df_cross_json']['data'][counter_i], function (k1, val2) {
...
// console.log('arr 700',arr)
// console.log('arr 700',Facts_keys)
// get selected value from facts_group_filter end here


		// html +='<tr class="hide_class"><th></th></tr><tr class="hide_class"><th></th></tr><tr class="hide_class"><th></th></tr>';
		// html +='<tr class="hide_class"><th colspan="0">Database </th><th>'+datbaseName[0]+'</th></tr>';
		// html +='<tr class="hide_class"><th colspan="0">Other Variable</th><th>'+other_variableArray+'</th></tr>';
		// html +='<tr class="hide_class"><th colspan="0">Dimensions </th><th>'+row_str+'</th></tr>';
		// html +='<tr class="hide_class"><th colspan="0">Filter </th>'+filter_data_text+'</tr>';
		// html +='<tr class="hide_class"><th colspan="0">Facts </th><th>'+JSON.stringify(arr)+'</th></tr>';
		// html +='<tr class="hide_class"><th colspan="0">Time Period </th><th>'+time_period_filter+'</th></tr>';
...
$("#toArrBtn").click(function(){
  ...
  setTimeout(function () {
	 	...
	 	var rowfilter_sort=exract_one_array_object_by_labels(r_text_val);
	 	var columnfilter_sort=exract_one_array_object_by_labels(c_text_val);
	 	// console.log('rowfilter_sort 800',rowfilter_sort)
	 	// console.log('columnfilter_sort 800',columnfilter_sort)
...
console.log('getSelectedValues_facts_filter',getSelectedValues_facts_filter);
	// ============================================================================
	// console.log('line 2740 filter data',filter_data)
	...
	$.each(get_tp_name, function (tp_key, tp_value) {
		...
		$.each(arr, function (facts_key, facts_values) {
			...
			html+='<h6 class="pl-2 mt-2">'+facts_key+'</h6>';
				$.each(facts_values, function (key1, value2) {
