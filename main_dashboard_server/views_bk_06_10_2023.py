...
def display_all_data(request):
    if request.method == "POST":
        ...
        df = pl.read_excel(settings.PYTHONPATH + filename+'.xlsx')
        df = df.to_pandas()
...
def crosstab_table(request):
    ...
    if request.method == "POST":
        ...
        selected_weight_column_all = ['Volume','Volume YA', 'Volume PP','Value (€)','Value YA (€)','Value PP (€)',
        ...
        for cross_df_col_loop in level_0_names_cols:
            print('====cross_df_col_loop 205', cross_df_col_loop)
            cross_df_col = cross_df_col_grp.get_group(cross_df_col_loop)
            cross_df_col_df = pd.DataFrame(cross_df_col)

        ############ renaming AAAAATotal to Grand Total ###########################################
            try:
                for level in cross_df.columns.levels:
                    if 'AAAATotal' in level:
                        cross_df_col_df = cross_df_col_df.rename(columns={'AAAATotal': 'Grand Total'},level=level.name)
            except:
                pass
            ...
            try:
                for level in cross_df.index.levels:
                    if 'AAAATotal' in level:
                        cross_df_col_df = cross_df_col_df.rename(index={'AAAATotal': 'Grand Total'},level=level.name)
            except:
                pass
        ############ renaming AAAAATotal to Grand Total ###########################################
            cross_df_dataframes_lst2 = []
            level_0_index = cross_df_col_df.index.get_level_values(0).unique().tolist()

            cross_df_index_grp = cross_df_col_df.groupby(axis=0, level=0)
            ...
            for cross_df_index_loop in level_0_index:
                ...
                cross_df_index_df = cross_df_index_df.droplevel(0, axis=1)
                filtered_df = filtered_df.droplevel(0, axis=1)

                cross_df_index_df = derived_measures_after_crosstab(cross_df_index_df, filtered_df)
                cross_df_index_df = pd.concat([cross_df_index_df], keys=[cross_df_col_loop], axis=1)
            ...
            cross_df_dataframes_lst.append(cross_df_temp)

        cross_df = pd.concat(cross_df_dataframes_lst, axis=1)
        cross_df.to_excel('558_CROSSDF.xlsx')
        # cross_df = cross_df.swaplevel(1, 0, axis=1).sort_index(axis=1)
        # cross_df = cross_df.reindex(columns=selected_weight_column_all, level=1)

        lst_measures,lst_measures_vals,dict_selected_measures_lst, = measures_facts_groups(selected_weight_column22,measure_selected_key_val_resp,crosstab_function_name)

        cross_df = cross_df.loc[:,cross_df.columns.get_level_values(1).isin(list(lst_measures_vals))]
        cross_df.to_excel('crosss_DFFF.xlsx')

        ############################### NEW LOGIC DERIVED COLUMNS - 29-09-2023 ############################

        print('lst_measures_vals====510',lst_measures_vals)
        if measure_under_time_flag == 0:

            cross_df = cross_df.swaplevel(0, 1, axis=1).sort_index(axis=1)
            print('measure_under_time_flag condn 1009==',measure_under_time_flag)
            cross_df.to_excel('cross_df_AFTER_SWAPPING.xlsx')
            cross_df = cross_df.reindex(columns=lst_measures_vals, level=0)
            # cross_df = cross_df.reindex(columns=cross_df.columns.levels[1][selected_weight_column_all], level=1)
        else:
            cross_df = cross_df.reindex(columns=lst_measures_vals, level=1)
            pass
            ...
            # cross_df = cross_df.reindex(columns=selected_weight_column_all, level=0)
            pass
            # cross_df = cross_df.reindex(columns=cross_df.columns.levels[0][selected_weight_column_all], level=0)
...
def crosstab_table_page2(request):
    ...
    ##################################### POLARS #########################################
        ...
        cross_df = pd.concat(cross_df_dataframes_lst, axis=1)
        cross_df.to_excel('cross_df_DDGFGFGF.xlsx')
        # cross_df = cross_df.swaplevel(1, 0, axis=1).sort_index(axis=1)
        # cross_df = cross_df.reindex(columns=selected_weight_column_all, level=1)
        ...
        ########### ADDED BY MIHIR PAWAR - 02-0S5-2023 INDICES ON COLUMN PERCENT #####
        if percent_calc == 'column_percent' and indices_calc_flag == 1:
            cross_df = Indices_selected_base_column(cross_df,base_column)
            percent_calc = 'Indices'
...
def bar_chart(request):
    ...
    ####################################################################################################
    filter_timeperiod = ['MAT']
    filter_fact = 'Volume'
    facts_groups_filter = 'Absolutes'
    dict_selected_measures_lst = {'Volume': ['Volume', 'Volume YA', 'Volume PP', 'Volume Share', 'Volume Share YA',
