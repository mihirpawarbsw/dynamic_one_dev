...
######################### ADDED ON 19-02-2025 - SORT NEW ############################################
def sort_columns_new_19_02_2025(cross_df, measure_row_column_position, column_to_sort, asc_desc_param):
    if measure_row_column_position == "measure_in_column":
        ...
        # Group by level 0 index only once
        ...
        for cross_df_index_loop in level_0_index:
            ...
            filtered_df = cross_df_index[cross_df_index.index.get_level_values(1).isin(['Grand Total', 'Total (Among Displayed)'])]
 
            # Check for 'cagr_none' in the sortable column (use vectorized operations)
            if cross_df_index[column_to_sort].str.contains('cagr_none', na=False).any():
                # Skip sorting if 'cagr_none' is found
                final_cross_df_index = pd.concat([cross_df_index, filtered_df])
            else:
                # Create a new column for sorting, handling percentages and non-percentage values
                sortable = cross_df_index[column_to_sort].apply(
                    lambda x: (float(x.replace('%', '')), True) if isinstance(x, str) and '%' in x else (x, False)
                )
 
                # Sort by the numeric values of the 'sortable' column
                cross_df_index['sortable'] = sortable
                cross_df_index.sort_values(by='sortable', key=lambda col: col.apply(lambda x: x[0]), ascending=asc_desc_param, inplace=True)
 
                # Restore original format for the sortable column
                cross_df_index[column_to_sort] = cross_df_index['sortable'].apply(
                    lambda x: f"{x[0]}%" if x[1] else x[0]
                )
                cross_df_index.drop(columns=['sortable'], inplace=True)
        ...
        # Return a copy when in row measure position
...
def derived_MAIN_fn(cross_df,base_sales_index_colname,row_name,col_name,measure_row_column_position,cagr_power_val,brand_sales_index_value_flag):
    # Convert to single-level DataFrame
