...
def display_all_data(request):
    if request.method == "POST":
        ...
        df = pd.read_csv(settings.TEMP_UPLOAD + filename+'.csv', encoding=detected_encoding)
        ##############################################################################
        ############################## OLD CODE ####################################
        ...
        colname_obj = df.select_dtypes(include=['object']).columns.tolist()
        colname_num = df.select_dtypes(include=['number']).columns.tolist()
...
def crosstab_table(request):
    ...
    if request.method == "POST":
        ...
        # Read the CSV file using the detected encoding
        df = pd.read_csv(settings.TEMP_UPLOAD + filename, encoding=detected_encoding)
        ##############################################################################
        ...
        if ('Category' in row_name) or ('Category' in col_name):
            mapping_category_dict = {'Skincare':'01}Skincare','Make-up':'02}Make-up','Fragrance':'03}Fragrance'}
            df['Category'] = df['Category'].replace(mapping_category_dict)
        ############### CODE TO ORDER CATEGORIES - 22-04-2024 #########################################

        ############### CODE TO ORDER CHANNEL - 26-04-2024 ##############################
        if ('Channel' in row_name) or ('Channel' in col_name):
            ...
            df['Channel'] = df['Channel'].replace(mapping_channel_dict)
        ############### CODE TO ORDER CHANNEL - 26-04-2024 #########################################

        # ############### CODE TO ORDER Brand - 26-04-2024 ###########################
        if ('Brand' in row_name) or ('Brand' in col_name):
            mapping_brand_dict = {'(Other panel)':'~}(Other panel)'}
            df['Brand'] = df['Brand'].replace(mapping_brand_dict)
        # ############### CODE TO ORDER Brand - 26-04-2024 ###########################
        ...
        if (db_flag != 'Doors') or ((db_flag == 'Doors') and ('Door' not in selected_weight_column22)):
            ...
            df,selected_full_period_str,comparative_full_period_str = data_transformation_doors(filename,selected_weight_column22,selected_full_period,comparative_full_period)
        ...
        ################### ADDED ON 20-06-2024 ####################################################
        if ("Brand" in row_name) and (len(row_name)==1) and (measure_row_column_position == 'measure_in_column'):
            unique_base_index_brand_product_lst = df['Brand'].unique().tolist()
            unique_base_index_brand_product_resp = {'Brand':unique_base_index_brand_product_lst}
        ...
        # Check if 'Category' key is present in the dictionary
        if 'Category' in filter_dict_resp:
            filter_dict_resp['Category'] = [re.sub(r'.*}', '', value) for value in filter_dict_resp['Category']]

        if 'Channel' in filter_dict_resp:
            filter_dict_resp['Channel'] = [re.sub(r'.*}', '', value) for value in filter_dict_resp['Channel']]

        if 'Brand' in filter_dict_resp:
            filter_dict_resp['Brand'] = [re.sub(r'.*}', '', value) for value in filter_dict_resp['Brand']]
        ################# CODE TO REPLACE "}" WITH "" -- 25-04-2024 ###
        ...
        if measure_row_column_position == "measure_in_row":
            ...
            cross_df = cross_df.T           
            cross_df = derived_MAIN_fn(cross_df,base_sales_index_colname,row_name,col_name,measure_row_column_position)
            cross_df = cross_df.T
        ...
        if (measure_row_column_position == 'measure_in_column') and (len(row_name) == 1) and (("Brand" in row_name) or ("Product Name" in row_name)):
            ...
            if len(selected_weight_column22) == 1:
                ...
                index_lst_cols = [0, 1, 3, 7, 8, 10, 14, 15, 17]
            elif len(selected_weight_column22) == 4:
                index_lst_cols = [0, 1, 3, 7, 8, 10, 14, 15, 17,21,22,24]
            elif len(selected_weight_column22) == 5:
                index_lst_cols = [0, 1, 3, 7, 8, 10, 14, 15, 17,21,22,24,28,29,31]
        ...
        if (('Category' in row_name) or ('Category' in col_name)) or (('Channel' in row_name) or ('Channel' in col_name)) or (('Brand' in row_name) or ('Brand' in col_name)):
            try:
...
def crosstab_table_page2(request):
    ...
    if request.method == "POST":
        ...
        print('cols thousand',df.columns)
        ...
        if (len(base_filter_col_lst)==0) and ((measure_row_column_position == 'measure_in_column') and (len(row_name) == 1) and (("Brand" in row_name) or ("Product Name" in row_name))):
            base_sales_index_colname = 'Shiseido'
            # print('1671!')

        elif 'Brand sales index' in base_filter_col_lst:
            ...
            df['Brand sales index'] = val_selected_brand_sales

            base_sales_index_colname = val_selected_brand_sales
        else:
            base_sales_index_colname = 'Shiseido'
            
        df.to_excel('df_ssshhhee.xlsx')
        # print('dict_base_filter_data 1696',dict_base_filter_data)
        # print('base_sales_index_colname 1697',base_sales_index_colname)
        # exit('base_filter_col_lst')
        # print('base_sales_index_colname 1675',base_sales_index_colname)
        # exit('base_filter_col_lst')
        ####################### added on 24-06-2024 #################################################
        ...
        #####################################################################################
            ...
            if not ((measure_row_column_position == 'measure_in_column') and (len(row_name) == 1) and (("Brand" in row_name) or ("Product Name" in row_name))):
            ...
            print('lst_measures_vals 1498',lst_measures_vals)
            ########################### NEW CODE - 29-05-2024 ###################################

            # measure_selected_key_val_resp = {}

            # for key in selected_weight_column22:
            #     measure_selected_key_val_resp[key] = [item for item in lst_measures_vals if item.startswith(key)]
            # print('measure_selected_key_val_resp1491',measure_selected_key_val_resp)
            # exit('ffffffdd')
            ################# NEW LOGIC FACTS - 10-05-2024 ############################
            ...
            # for key in selected_weight_column22:
            #     dict_selected_measures_lst[key] = [item for item in last_level_values_list if item.startswith(key)]
            # # print('last_level_values_list111',last_level_values_list)
            # # print('dict_selected_measures_lst2222',dict_selected_measures_lst)

            # # ################## CODE TO REPLACE CY AND YA WITH ACTUAL PERIOD - 19-04-2024 ####
            # period_to_be_replaced_str = [selected_full_period_str][0].split()[0]
            # print('measure_selected_key_val_resp 1351',measure_selected_key_val_resp)
            # ###### CODE TO REPLACE TUM PERIODS FOR ALIGNMENT 19-04-2024 #############
            # # Define your dictionary and predefined list
            # predefined_list_TIME = ['QUARTER', 'YTD', 'MAT','HY','FY']

            # # Iterate through the dictionary values
            # for key, values in measure_selected_key_val_resp.items():
            #     for i in range(len(values)):
            #         # Iterate through the predefined list
            #         for period_str in predefined_list_TIME:
            #             # Check if the period string is present in the value
            #             if period_str in values[i]:
            #                 # Replace the matched string with the new string
            #                 values[i] = values[i].replace(period_str, period_to_be_replaced_str)

            # print('measure_selected_key_val_resp 14333',measure_selected_key_val_resp)
            # # print('dict_selected_measures_lst 1433333333444',dict_selected_measures_lst)

            # # if (any('Sales (LC)' in item for item in selected_weight_column_all)):
            # #     sublist_LC = [last_level_values_list[0], 'Sales Growth vs']
            # #     dict_selected_measures_lst[0] = sublist_LC

            # # print('dict_selected_measures_lst 32221',dict_selected_measures_lst)

            #  ###### CODE TO REPLACE TUM PERIODS FOR ALIGNMENT 19-04-2024 #############

            # ########################### CODE FOR FILTERED FACT RESP - 19-04-2024 #############
            # lst_measures = list(measure_selected_key_val_resp.values())
            # print('lst_measures 1379',lst_measures)
            # lst_measures_vals = [item for sublist in lst_measures for item in (sublist if isinstance(sublist, list) else [sublist])]
            # print('lst_measures 11577',lst_measures_vals)
            # # ############################################ ################################
            # # ############################################ ################################ 
            # print('selected_full_period_str 134555',selected_full_period_str)
            # print('comparative_full_period_str 1444',comparative_full_period_str)

            # cy_lst_selected = [elem for elem in lst_measures_vals if selected_full_period_str in elem]
            # print('cy_lst_selected',cy_lst_selected)
            # ya_lst_selected = [elem for elem in lst_measures_vals if comparative_full_period_str in elem]
            # print('ya_lst_selected',ya_lst_selected)
            # cy_ya_comb = cy_lst_selected + ya_lst_selected
            # print('cy_ya_comb',cy_ya_comb)

            # # Convert lists to sets
            # larger_set = set(lst_measures_vals)
            # smaller_set = set(cy_ya_comb)

            # print('larger_set=',larger_set)
            # print('smaller_set=',smaller_set)

            # # Get elements from larger set that are not in smaller set
            # result_set = larger_set - smaller_set

            # # Convert result back to list
            # not_common_cy_ya = list(result_set)
            # print('not_common_cy_ya',not_common_cy_ya)

            # # # ###########################################################################
            # # Iterate over each element in the original list
            # if len(not_common_cy_ya) > 0:
            #     modified_list = []
            #     for elem in not_common_cy_ya:
            #         # Find the position of "QUARTER"
            #         quarter_index = elem.find(period_to_be_replaced_str)
            #         if quarter_index != -1:
            #             # Replace the text after "QUARTER" with the new string
            #             modified_elem = elem[:quarter_index] + comparative_full_period_str
            #             modified_list.append(modified_elem)
            #         else:
            #             # If "QUARTER" is not found, keep the original element
            #             modified_list.append(elem)
            # else:
            #     modified_list = []

            # final_cy_ya_list = cy_lst_selected + ya_lst_selected + modified_list
            # print('final_cy_ya_list',final_cy_ya_list)

            # # Get the key with the list value
            # key_with_list = next(iter(measure_selected_key_val_resp))

            # # Replace the list value with the new list
            # measure_selected_key_val_resp[key_with_list] = final_cy_ya_list

            # lst_measures = list(measure_selected_key_val_resp.values())
            # lst_measures_vals = [item for sublist in lst_measures for item in (sublist if isinstance(sublist, list) else [sublist])]
            # print('FINAL lst_measures_vals',lst_measures_vals)
            # ###########################################################################
            # ############################################ ################################
            # ############################################ ################################
...
def current_time_period_resp(request):
    if request.method == "POST":
        ...
        # Read the CSV file using the detected encoding
        data = pd.read_csv(settings.TEMP_UPLOAD + filename, encoding=detected_encoding)
        ##############################################################################
        df = data.copy()
        ...
        print('305 time_period_dict', time_period_dict)
...
def comparative_time_period_resp(request):
    ...
    ################################### added on 06-05-2024 #################################       
        ...
        print('305 time_period_dict BEFORE', time_period_dict)
        ########################################### added on 06-05-2024 ##############################
