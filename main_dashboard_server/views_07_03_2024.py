            ...
            if len(row_name) == 1:
                ...
                if selected_weight_column in lst_nd_wd_cols:
                    ...
                    df_level1.index = df_level1.index = pd.MultiIndex.from_arrays([df_level1.index.values,
                                                                                   len(df_level1.index) * ['000AAAATotal']])
                else:
        ...
        ############ renaming AAAAATotal to Grand Total ###########################################
            try:
                for level in cross_df.columns.levels:
                    if '000AAAATotal' in level:
                        cross_df_col_df = cross_df_col_df.rename(columns={'000AAAATotal': 'Grand Total'},level=level.name)
            except:
                pass
            ...
            try:
                for level in cross_df.index.levels:
                    if '000AAAATotal' in level:
                        cross_df_col_df = cross_df_col_df.rename(index={'000AAAATotal': 'Grand Total'},level=level.name)
            except:
                pass
        ############ renaming AAAAATotal to Grand Total ###########################################
