...
def crosstab_table_page2(request):
    ...
    if request.method == "POST":
        ...
        if 'Brand sales index' in base_filter_col_lst and 'Brand' in base_filter_col_lst:
        ...
        #####################################################################################
            ...
            if (display_grand_total_flag == 'gt_all') and ((len(col_name)==0 and len(row_name)==1) or (len(col_name)==1 and len(row_name)==1)):
                ...
                cross_df = cross_df.copy()
            # cross_df.to_excel('formatted_cross_df.xlsx')
            ############ ADD UNFILTERED Grand Total to rows - 04102024 ########################
            ############ ADD UNFILTERED Grand Total to rows - 04102024 ########################
            ...
            for key, indexes in facts_object_index_resp.items():
                ...
                if (8 in indexes) and ('Brand sales index' not in filter_dict_resp.keys()):
        ...
        #### CODE TO DROP GRAND TOTAL WHEN SALES LC  IS SELECTED - 18-04-2024 ####
            ...
            # save_file_path = r"\var\www\html\bsstracker\BCST_Sales_Tool_demo\static\download_cross_table_excel\\"
            # cross_df.to_excel(save_file_path+'CROSSDF_FINALL.xlsx')
