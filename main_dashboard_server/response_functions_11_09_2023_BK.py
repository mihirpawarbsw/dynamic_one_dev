...
def base_filter_resp(df, dict_base_filter, row_name, col_name,selected_weight_column_all):
...
def saving_crosstab_excel_file(cross_df, legends_df, percent_calc):
    ...
    for row in sheet.iter_rows():
        for cell in row:
            # Check if the cell contains "AAAATotal"
            if isinstance(cell.value, str) and "AAAATotal" in cell.value:
                # Replace "AAAATotal" with "Grand Total"
                cell.value = cell.value.replace("AAAATotal", "Grand Total")
